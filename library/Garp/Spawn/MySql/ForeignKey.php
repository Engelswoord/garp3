...
class Garp_Spawn_MySql_ForeignKey extends Garp_Spawn_MySql_Key {
    ...
    public static function renderSqlDefinition($fkName, $columnName, $modelName, $relType) {
        ...
        $events = match ($relType) {
            'belongsTo', 'hasAndBelongsToMany' => 'ON DELETE CASCADE ON UPDATE CASCADE',
            default => 'ON DELETE SET NULL ON UPDATE CASCADE',
        };
    ...
    static public function isForeignKeyStatement($line) {
        return stripos((string) $line, 'CONSTRAINT') !== false;
    }


    protected function _parse($line) {
        $matches = [];
        preg_match('/\s*CONSTRAINT\s+`(?P<name>\w+)`\s+FOREIGN KEY\s*\(`(?P<localColumn>\w+)`\)\s+REFERENCES\s+`(?P<remoteTable>\w+)`\s+\(`(?P<remoteColumn>\w+)`\)\s*(?P<events>[\w\s]+)?/i', trim((string) $line), $matches);
        return $matches;
    }
}
